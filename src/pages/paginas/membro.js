...
function Membro() {
   ...
   const buscarLogin = (id2) => {
        Axios.post('https://avaliacao-360.herokuapp.com/api/buscarUser', {
            ...
            if(!( ( response.data[0].idpessoa==id ) || ( response.data[0].diretor==1 && response.data[0].referenciaidsubsistema==user.referenciaidsubsistema ) || ( response.data[0].diretorGeral==1 && response.data[0].sistemaID==user.sistemaID ) || ( response.data[0].capitao==1 ) ) ){
